...
function MenuEditor(idSelector, options) {
    ...
    var settings = {
        ...
        listOptions: { 
            ...
            opener: {
                ...
                openerClass: 'btn btn-success btn-sm-cus',
            },
    ...
    $main.on('click', '.btnOut', function (e) {
        ...
        var Parentid = $liParent.data("parentid");
        ...
        MenuEditor.updateButtons($main);
        $li.attr("data-parentid", Parentid);
    });
    ...
    function TButtonGroup() {
        ...
        var $btnEdit = TButton({classCss: 'btn btn-primary btn-sm-cus btnEdit', text: settings.labelEdit});
        var $btnRemv = TButton({classCss: 'btn btn-danger btn-sm-cus btnRemove', text: settings.labelRemove});
        var $btnUp = TButton({classCss: 'btn btn-iconmenu btn-sm-cus btnUp btnMove', text: '<i class="fas fa-angle-up clickable"></i>'});
        var $btnDown = TButton({classCss: 'btn btn-iconmenu btn-sm-cus btnDown btnMove', text: '<i class="fas fa-angle-down clickable"></i>'});
        var $btnOut = TButton({classCss: 'btn btn-iconmenu btn-sm-cus btnOut btnMove', text: '<i class="fas fa-level-down-alt clickable"></i>'});
        var $btnIn = TButton({classCss: 'btn btn-iconmenu btn-sm-cus btnIn btnMove', text: '<i class="fas fa-level-up-alt clickable"></i>'});
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var $li = $('<li>').addClass('list-group-item pr-0 pt-2 pb-1');
            $li.data(itemObject);
            $li.attr("data-id", v.LienKetID);
            $li.attr("data-slug", v.href);
            $li.attr("data-taget", v.target);
            $li.attr("data-icon", v.icon);
    ...
    this.update = function(){
        ...
        $cEl.attr("data-id", Common.NewGuild());
        $cEl.attr("data-slug", $cEl.data('href'));
        $cEl.attr("data-taget", $cEl.data('target'));
        $cEl.attr("data-icon", $cEl.data('icon'));
        resetForm();
    };
    ...
    this.add = function(){
        ...
        var $li = $("<li>").data(data);
        $li.addClass('list-group-item pr-0  pt-2 pb-1').append(div);
        $li.attr("data-id", Common.NewGuild());
        $li.attr("data-slug", data.href);
        $li.attr("data-taget", data.target);
        $li.attr("data-icon", data.icon);
        $main.append($li);
        MenuEditor.updateButtons($main);
        resetForm();
    };
