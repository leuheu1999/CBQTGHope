var BC_CongBao = function (options) {
    for (var property in options) {
        this[property] = options[property];
    }
    return this.Initialize(options);
}
BC_CongBao.prototype = {
    ...
    RegisterEvent: function () {
        ...
        $("a[data-action='reload']").unbind("click").click(function () {
            ...
            Common.BC_CongBao.SubmitForm();
        });

        $("#btnTimKiem").unbind("click").click(function () {
            Common.BC_CongBao.SetPage(1);
            Common.BC_CongBao.SubmitForm();
            return false;
        });
        $("#QuyenID").unbind("change").change(function () {
            ...
            Common.BC_CongBao.AjaxCBOLoaiHinh(val);
            return false;
        });
        $("#KyBaoCao").unbind("select2:select").on("select2:select", (e) => {
            ...
            Common.BC_CongBao.RefeshSelect2("#Nam, #Quy, #Thang");
        });
        $("#btn-word").unbind("click").click(function () {
            ...
            Common.Ajax({
                type: "POST",
                url: Common.BC_CongBao.Url.ExportWordPdf,
                async: false,
                cache: false,
                data: { Param: _model, Key: 'ExportBC_CongBaoList_Word' }
            }, function (result) {
                if (result) {
        ...
        $("#btn-excel").unbind("click").click(function () {
            ...
            Common.Ajax({
                type: "POST",
                url: Common.BC_CongBao.Url.ExportExcel,
                async: false,
                cache: false,
                data: { Param: _model, Key: 'ExportBC_CongBaoList_Excel' }
            }, function (result) {
                if (result) {
                    let a = document.createElement('a')
                    a.href = 'data:application/octet-stream;base64,' + result.data;
        ...
        $("#btn-pdf").unbind("click").click(function () {
            ...
            Common.Ajax({
                ...
                async: false,
                cache: false,
                data: { Param: _model, Key: 'ExportBC_CongBaoList_Pdf' }
            }, function (result) {
                if (result) {
                    let a = document.createElement('a')
                    a.href = 'data:application/octet-stream;base64,' + result.data;
    ...
    Paging: function (page) {
        Common.BC_CongBao.SetPage(page);
        Common.BC_CongBao.SubmitForm();
        return false;
    },
    ...
    ChangePageSize: function (e) {
        Common.BC_CongBao.SetPageSize($(e).val());
        Common.BC_CongBao.SubmitForm();
        return false;
    },
    ...
    Success: function (data) {
        ...
        Common.BC_CongBao.RegisterEvent();
    },
    AjaxCBOLoaiHinh: function (id) {
        Common.Ajax({
            url: Common.BC_CongBao.Url.CBOLoaiHinhById,
            type: "POST",
            ...
            if (!Common.Empty(result)) {
                var html = Common.BC_CongBao.TemplateHtmlCBO("Loại hình", "LoaiHinhID", "LoaiHinhID", result.data);
...
// create BC_CongBao Object
var BC_CongBao = BC_CongBao;
BC_CongBao.constructor = BC_CongBao;
