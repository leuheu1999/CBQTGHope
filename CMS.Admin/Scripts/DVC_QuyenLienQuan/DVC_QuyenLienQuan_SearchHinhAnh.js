var DVC_QuyenLienQuan_SearchHinhAnh = function (options) {
    for (var property in options) {
        this[property] = options[property];
    }
    return this.Initialize(options);
}
DVC_QuyenLienQuan_SearchHinhAnh.prototype = {
    ...
    RegisterEvent: function () {
        ...
        $("#btnAddNewRowHA").unbind("click").click(function (event) {
            ...
            tr.innerHTML = Common.DVC_QuyenLienQuan_SearchHinhAnh.AddTrInTable(numberCurrent);
        ...
        $(".btn-update-hinhanh").unbind("click").click(function () {
            ...
            Common.DVC_QuyenLienQuan_SearchHinhAnh.BindDataUpdate(tr, Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh[id]);
            return false;
            ...
            //    Common.Ajax({
            //        url: Common.DVC_QuyenLienQuan_SearchHinhAnh.Url.UpdateHinhAnh,
            //        type: "POST",
            ...
            //                    Common.DVC_QuyenLienQuan_SearchHinhAnh.BindDataUpdate(tr, result.data);
            //                    return false;
            //                }
            //            }
            //            else {
        ...
        $(".btn-xoa-hinhanh").unbind("click").click(function () {
            ...
            Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh.splice(id, 1);
            Common.DVC_QuyenLienQuan_SearchHinhAnh.SubmitForm();
            return false;
            //var quyenLienQuanID = $("#form-update-tt-tacpham").find("input[name='QuyenLienQuanID']").val() || 0;
            //if (quyenLienQuanID == 0) {
            //    Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh.shift(id - 1);
            //    Common.DVC_QuyenLienQuan_SearchHinhAnh.SubmitForm();
            //    return false;
            //}
            //else {
            //    Common.ShowLoading(true);
            //    Common.Ajax({
            //        url: Common.DVC_QuyenLienQuan_SearchHinhAnh.Url.DeleteHinhAnh,
            //        type: "POST",
            //        ContentType: 'application/json; charset=utf-8',
            //        async: false,
            //        cache: false,
        ...
        $('.btn-chon-hinhanh').unbind("click").click(function () {
            ...
            $("#HinhDaiDienUrl").attr("src", src);
            Common.DVC_QuyenLienQuan_SearchHinhAnh.HideDialog();
            return false;
        });
    },
    SubmitForm: function () {
        ...
        if (keyPage == 'update') {
            if (click >= 1) {
                var listDuLieu = Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh;
                var html = '';
                ...
                Common.DVC_QuyenLienQuan_SearchHinhAnh.ShowDialog();
                return false;
            }
            else {
                ...
                Common.Ajax({
                    type: 'POST',
                    url: Common.DVC_QuyenLienQuan_SearchHinhAnh.Url.SearchHinhAnh,
                    data: formdata,
                    ...
                    $("#tableUpdateHinhAnh tbody tr").each(function () {
                        if ($(this).closest("tr").data("id") >= 0) {
                            Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh.push({
    ...
    Paging: function (page) {
        Common.DVC_QuyenLienQuan_SearchHinhAnh.SetPage(page);
        Common.DVC_QuyenLienQuan_SearchHinhAnh.SubmitForm();
        return false;
    },
    ...
    ChangePageSize: function (e) {
        Common.DVC_QuyenLienQuan_SearchHinhAnh.SetPageSize($(e).val());
        Common.DVC_QuyenLienQuan_SearchHinhAnh.SubmitForm();
        return false;
    },
    ShowDialog: function () {
        $("#dialogSearchHinhAnh").modal("show")
        Common.DVC_QuyenLienQuan_SearchHinhAnh.RegisterEvent();
    },
    HideDialog: function () {
    ...
    AddTrInTable: function (stt) {
        ...
        html += '<td class="text-center file-field" >'
            + '<button type = "button" class="btn btn-primary btn-block-sm btn-sm mt-md-3 mt-sm-3 uploadicon" onclick = "Common.DVC_QuyenLienQuan_SearchHinhAnh.UploadFileClick(this)"><i class="fas fa-upload"></i></button>'
        ...
        html += '<button title="lưu" class="btn btn-success btn-sm mr-1" type="button" onclick="' + "Common.DVC_QuyenLienQuan_SearchHinhAnh.SaveTrData(this," + stt + ",'Save')" + '"><i class="far fa-save"></i></button>';
        html += '<button title="hủy" class="btn btn-danger btn-sm" type="button" onclick="Common.DVC_QuyenLienQuan_SearchHinhAnh.DeleteTrData(this)"><i class="fa fa-times"></i></button>';
        html += '</td>';
    ...
    EditTrInTable: function (item, stt) {
        ...
        html += '<td class="text-center cursor" data-label="Thao tác">';
        html += '<button title="lưu" class="btn btn-success btn-sm mr-1" type="button" onclick="' + "Common.DVC_QuyenLienQuan_SearchHinhAnh.SaveTrData(this," + stt + ",'Save')" + '"><i class="far fa-save"></i></button>';
        html += '<button title="Xóa" class="btn btn-danger btn-sm btn-xoa-hinhanh" type="button" data-id="' + stt + '" onclick="Common.DVC_QuyenLienQuan_SearchHinhAnh.DeleteData(' + stt + ')"><i class="far fa-trash-alt"></i></button>';
        html += '</td>';
        return html;
    },
    UploadFileClick: function (that) {
        ...
        $('.form-control-file').unbind("change").change(function (ev) {
            ev.preventDefault();
            Common.DVC_QuyenLienQuan_SearchHinhAnh.UploadFileChange(this);
            return false;
        });
    },
    UploadFileChange: function (ev) {
        ...
        if (fileUploads.length > 0) {
            ...
            var sizeMB = 20;
            if (!regex.test(Common.DVC_QuyenLienQuan_SearchHinhAnh.GetTypeFile(fileName))) {
                $.notify({
                    // options
                ...
                Common.Ajax({
                    type: "POST",
                    url: Common.DVC_QuyenLienQuan_SearchHinhAnh.Url.UploadPicture,
                    dataType: 'json',
                    data: data,
                    contentType: false, // Not to set any content header
                    processData: false, // Not to process data
    ...
    SaveTrData: function (that, stt, action) {
        ...
        if (Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh[stt]) {
            Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh[stt].Ten = $.trim(tr.find("input[name='" + stt + "Ten']").val()) || '';
            Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh[stt].CreatedUser = $.trim(tr.find("input[name='" + stt + "CreatedUser']").val()) || '';
            Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh[stt].Tag = $.trim(tr.find("textarea[name='" + stt + "Tag']").val()) || '';
            Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh[stt].GhiChu = $.trim(tr.find("textarea[name='" + stt + "GhiChu']").val()) || '';
            Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh[stt].CreatedDate = $.trim(tr.find("input[name='" + stt + "CreatedDate']").val()) || '';
            Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh[stt].TenTepTin = div.data("tenfile") || '';
            Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh[stt].TenGoc = div.data("tenfilegoc") || '';
            Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh[stt].DuongDan = div.data("duongdan") || '';
        }
        else {
            Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh.push({
        ...
        //if (Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh[stt])
        //    Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh.splice(stt, 1);
        //Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh.push({
        //    Ten: $.trim(tr.find("input[name='" + stt + "Ten']").val()) || '',
        //    CreatedUser: $.trim(tr.find("input[name='" + stt + "CreatedUser']").val()) || '',
        //    Tag: $.trim(tr.find("textarea[name='" + stt + "Tag']").val()) || '',
        ...
        return false;
        //if (quyenLienQuanID == 0) {
        //    if (Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh[stt-1])
        //        Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh.shift(stt);
        //    Common.DVC_QuyenLienQuan_SearchHinhAnh.ListHinhAnh.push({
        //        Ten: $.trim(tr.find("input[name='" + stt + "Ten']").val()) || '',
        //        CreatedUser: $.trim(tr.find("input[name='" + stt + "CreatedUser']").val()) || '',
        //        Tag: $.trim(tr.find("textarea[name='" + stt + "Tag']").val()) || '',
        ...
        //    $.ajax({
        //        type: 'POST',
        //        url: Common.DVC_QuyenLienQuan_SearchHinhAnh.Url.SaveHinhAnh,
        //        data: formdata,
        //        datatype: "json",
        //        contentType: false, // Not to set any content header
        //        processData: false, // Not to process data
    ...
    BindDataUpdate: function (tr, model) {
        ...
        //add td
        let html = new Array();
        let row = Common.DVC_QuyenLienQuan_SearchHinhAnh.EditTrInTable(model, numberCurrent);
        html.push(row);
        tr.append(html);
...
var DVC_QuyenLienQuan_SearchHinhAnh = DVC_QuyenLienQuan_SearchHinhAnh;
DVC_QuyenLienQuan_SearchHinhAnh.constructor = DVC_QuyenLienQuan_SearchHinhAnh;
