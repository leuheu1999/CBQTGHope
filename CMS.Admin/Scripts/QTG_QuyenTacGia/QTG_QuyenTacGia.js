var QTG_QuyenTacGia = function (options) {
    for (var property in options) {
        this[property] = options[property];
    }
    return this.Initialize(options);
};
QTG_QuyenTacGia.prototype = {
    ...
    AgainRegister: function () {
        $("a[data-action='reload']").unbind("click").click(function () {
            ...
            Common.QTG_QuyenTacGia.SetPage(1);
            Common.QTG_QuyenTacGia.SubmitSearchForm();
        });
        $("#btn-exportWord").unbind("click").click(function () {
            ...
            Common.Ajax({
                type: "POST",
                url: Common.QTG_QuyenTacGia.Url.ExportWordPdf,
                async: false,
                cache: false,
                data: { Param: _model, Key: 'ExportQTG_QuyenTacGiaList_Word' }
            }, function (result) {
                if (result) {
        ...
        $("#btn-exportExcel").unbind("click").click(function () {
            ...
            Common.Ajax({
                type: "POST",
                url: Common.QTG_QuyenTacGia.Url.ExportExcel,
                async: false,
                cache: false,
                data: { Param: _model, Key: 'ExportQTG_QuyenTacGiaList_Excel' }
            }, function (result) {
                if (result) {
                    let a = document.createElement('a')
                    a.href = 'data:application/octet-stream;base64,' + result.data;
        ...
        $("#btn-exportPdf").unbind("click").click(function () {
            ...
            Common.Ajax({
                ...
                async: false,
                cache: false,
                data: { Param: _model, Key: 'ExportQTG_QuyenTacGiaList_Pdf' }
            }, function (result) {
                if (result) {
                    let a = document.createElement('a')
                    a.href = 'data:application/octet-stream;base64,' + result.data;
                ...
                Items: {
                    Cancel: {
                        ...
                        OnClick: function (target) {
                            ...
                            Common.Ajax({
                                type: "POST",
                                url: Common.QTG_QuyenTacGia.Url.DeleteQuyenTacGia,
                                async: false,
                                cache: false,
                                data: {
                                    id: id
    ...
    Paging: function (page) {
        Common.QTG_QuyenTacGia.SetPage(page);
        Common.QTG_QuyenTacGia.SubmitSearchForm();
        return false;
    },
    ...
    ChangePageSize: function (e) {
        Common.QTG_QuyenTacGia.SetPageSize($(e).val());
        Common.QTG_QuyenTacGia.SubmitSearchForm();
        return false;
    },
    ...
    BeforeSend: function () {
        ...
        if (this.nodeName == "A") {
            page = Common.QTG_QuyenTacGia.GetCurrentPage($(this));
            Common.QTG_QuyenTacGia.SetPage(page);
            return false;
        } else {
            Common.QTG_QuyenTacGia.SetPage(1);
    ...
    Success: function () {
        ...
        Common.QTG_QuyenTacGia.RegisterEvent();
        Common.QTG_QuyenTacGia.AgainRegister();
    },
    SetPage: function (page) {
        $("#search-form").find("input[name='PageIndex']").val(page);
    },
};
var QTG_QuyenTacGia = QTG_QuyenTacGia;
QTG_QuyenTacGia.constructor = QTG_QuyenTacGia;