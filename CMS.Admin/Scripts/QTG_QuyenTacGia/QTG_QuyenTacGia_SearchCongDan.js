var QTG_QuyenTacGia_SearchCongDan = function (options) {
    for (var property in options) {
        this[property] = options[property];
    }
    return this.Initialize(options);
}
QTG_QuyenTacGia_SearchCongDan.prototype = {
    ...
    RegisterEvent: function () {
        ...
        $("#btnAddCongDan").unbind("click").click(function (event) {
            ...
            if ($('input[name="chkInputCheckedCSH[]"]:checked').length > 0) {
                $('input[name="chkInputCheckedCSH[]"]').each(function () {
                    if ($(this).prop("checked")) {
                        var check = Common.QTG_QuyenTacGia_SearchCongDan.ListChuSoHuu.find(x => x.ChuSoHuuID === $(this).closest("tr").data("id"));
            ...
            if ($('input[name="chkInputCheckedTG[]"]:checked').length > 0) {
                $('input[name="chkInputCheckedTG[]"]').each(function () {
                    if ($(this).prop("checked")) {
                        var check = Common.QTG_QuyenTacGia_SearchCongDan.ListTacGia.find(x => x.TacGiaID === $(this).closest("tr").data("id"));
                        //if (typeof check != 'undefined') {
                        //    $.notify({
                        //        // options
            ...
            if (key == 'ChuyenChu')
                Common.QTG_QuyenTacGia_SearchCongDan.LoadTableChuSoHuu();
            else if (key == 'CapDoi')
                Common.QTG_QuyenTacGia_SearchCongDan.LoadTableTacGia();
            else {
                Common.QTG_QuyenTacGia_SearchCongDan.LoadTableChuSoHuu();
                Common.QTG_QuyenTacGia_SearchCongDan.LoadTableTacGia();
            }
            Common.QTG_QuyenTacGia_SearchCongDan.HideDialog();
            return false;
        });
        ...
        $(".btn-update-congdan").unbind("click").click(function () {
            ...
            Common.Ajax({
                url: Common.QTG_QuyenTacGia_SearchCongDan.Url.UpdateCongDan,
                type: "POST",
        ...
        $("#btnSaveCongDan").unbind("click").click(function () {
            ...
            if ($("#form-update-congdan").find(".field-validation-error").length == 0) {
                ...
                $.ajax({
                    type: 'POST',
                    url: Common.QTG_QuyenTacGia_SearchCongDan.Url.SaveCongDan,
                    data: formdata,
                    datatype: "json",
    ...
    SubmitForm: function () {
        ...
        Common.Ajax({
            type: 'POST',
            url: Common.QTG_QuyenTacGia_SearchCongDan.Url.SearchCongDan,
            data: formdata,
            contentType: false, // Not to set any content header
            processData: false, // Not to process data
            dataType: 'html',
            ...
            Common.QTG_QuyenTacGia_SearchCongDan.ShowDialog();
            $('input[name="chkInputCheckedCSH[]"]').each(function () {
                var check = Common.QTG_QuyenTacGia_SearchCongDan.ListChuSoHuu.find(x => x.ChuSoHuuID === $(this).closest("tr").data("id"));
                if (typeof check != 'undefined') {
    ...
    Paging: function (page) {
        Common.QTG_QuyenTacGia_SearchCongDan.SetPage(page);
        Common.QTG_QuyenTacGia_SearchCongDan.SubmitForm();
        return false;
    },
    ...
    ChangePageSize: function (e) {
        Common.QTG_QuyenTacGia_SearchCongDan.SetPageSize($(e).val());
        Common.QTG_QuyenTacGia_SearchCongDan.SubmitForm();
        return false;
    },
    ShowDialog: function () {
    ...
    LoadTableChuSoHuu: function () {
        var html = '';
        if (Common.QTG_QuyenTacGia_SearchCongDan.ListChuSoHuu != null && Common.QTG_QuyenTacGia_SearchCongDan.ListChuSoHuu.length > 0) {
            for (i = 0; i < Common.QTG_QuyenTacGia_SearchCongDan.ListChuSoHuu.length; i++) {
                var chuSoHuu = Common.QTG_QuyenTacGia_SearchCongDan.ListChuSoHuu[i];
                html += '<tr>';
                html += '<td class"cursor" onclick="Common.QTG_QuyenTacGia_SearchCongDan.DetailCongDan(' + chuSoHuu.ChuSoHuuID + ')" data-label="Họ tên">' + chuSoHuu.HoVaTen + '</td>';
                ...
                html += '<button title="Xóa chủ sở hữu" data-id="' + chuSoHuu.ChuSoHuuID + '" class="btn btn-danger btn-sm" type="button" onclick="Common.QTG_QuyenTacGia_SearchCongDan.RemovedChuSoHuu(' + i + ')"><i class="far fa-trash-alt"></i></button>';
                html += '</td>';
                html += '</tr>';
            }
        }
    ...
    LoadTableTacGia: function () {
        var html = '';
        if (Common.QTG_QuyenTacGia_SearchCongDan.ListTacGia != null && Common.QTG_QuyenTacGia_SearchCongDan.ListTacGia.length > 0) {
            for (i = 0; i < Common.QTG_QuyenTacGia_SearchCongDan.ListTacGia.length; i++) {
                var tacGia = Common.QTG_QuyenTacGia_SearchCongDan.ListTacGia[i];
                html += '<tr>';
                html += '<td class="cursor" onclick="Common.QTG_QuyenTacGia_SearchCongDan.DetailCongDan(' + tacGia.TacGiaID + ')" data-label="Họ tên">' + tacGia.HoVaTen + '</td>';
                ...
                html += '<td data-label="Thao tác" class="text-center">';
                html += '<button title="Xóa tác giả" data-id="' + tacGia.TacGiaID + '" class="btn btn-danger btn-sm" type="button" onclick="Common.QTG_QuyenTacGia_SearchCongDan.RemovedTacGia(' + i + ')"><i class="far fa-trash-alt"></i></button>';
                html += '</td>';
                html += '</tr>';
            }
        }
    ...
    RemovedChuSoHuu: function (id) {
        Common.QTG_QuyenTacGia_SearchCongDan.ListChuSoHuu.splice(id, 1)
        Common.QTG_QuyenTacGia_SearchCongDan.LoadTableChuSoHuu();
        return false;
    },
    RemovedTacGia: function (id) {
        Common.QTG_QuyenTacGia_SearchCongDan.ListTacGia.splice(id, 1)
        Common.QTG_QuyenTacGia_SearchCongDan.LoadTableTacGia();
        return false;
    },
};

var QTG_QuyenTacGia_SearchCongDan = QTG_QuyenTacGia_SearchCongDan;
QTG_QuyenTacGia_SearchCongDan.constructor = QTG_QuyenTacGia_SearchCongDan;
