var QTG_QuyenTacGia_SearchPhim = function (options) {
    for (var property in options) {
        this[property] = options[property];
    }
    return this.Initialize(options);
}
QTG_QuyenTacGia_SearchPhim.prototype = {
    ...
    RegisterEvent: function () {
        ...
        $("#btnAddNewRowP").unbind("click").click(function (event) {
            ...
            tr.innerHTML = Common.QTG_QuyenTacGia_SearchPhim.AddTrInTable(numberCurrent);
        ...
        $(".btn-update-phim").unbind("click").click(function () {
            ...
            Common.QTG_QuyenTacGia_SearchPhim.BindDataUpdate(tr, Common.QTG_QuyenTacGia_SearchPhim.ListPhim[id]);
            return false;
            //Common.ShowLoading(true);
            //Common.Ajax({
            //    url: Common.QTG_QuyenTacGia_SearchPhim.Url.UpdatePhim,
            //    type: "POST",
            ...
            //                Common.QTG_QuyenTacGia_SearchPhim.BindDataUpdate(tr, result.data);
            //                return false;
            //            }
            //        }
            //        else {
        ...
        $(".btn-xoa-phim").unbind("click").click(function () {
            ...
            Common.QTG_QuyenTacGia_SearchPhim.ListPhim.splice(id, 1);
            Common.QTG_QuyenTacGia_SearchPhim.SubmitForm();
            return false;
            //Common.ShowLoading(true);
            //Common.Ajax({
            //    url: Common.QTG_QuyenTacGia_SearchPhim.Url.DeletePhim,
            //    type: "POST",
            //    ContentType: 'application/json; charset=utf-8',
            //    async: false,
            //    cache: false,
        ...
        $('.btn-chon-phim').unbind("click").click(function () {
            ...
            //$("#PhimID").val(id);
            $("#TenPhim").val(name);
            Common.QTG_QuyenTacGia_SearchPhim.HideDialog();
            return false;
        });
    },
    SubmitForm: function () {
        ...
        if (keyPage == 'update') {
            if (click >= 1) {
                var listDuLieu = Common.QTG_QuyenTacGia_SearchPhim.ListPhim;
                var html = '';
                ...
                Common.QTG_QuyenTacGia_SearchPhim.ShowDialog();
                return false;
            }
            else {
                ...
                Common.Ajax({
                    type: 'POST',
                    url: Common.QTG_QuyenTacGia_SearchPhim.Url.SearchPhim,
                    data: formdata,
                    ...
                    $("#tableUpdatePhim tbody tr").each(function () {
                        if ($(this).closest("tr").data("id") >= 0) {
                            Common.QTG_QuyenTacGia_SearchPhim.ListPhim.push({
    ...
    Paging: function (page) {
        Common.QTG_QuyenTacGia_SearchPhim.SetPage(page);
        Common.QTG_QuyenTacGia_SearchPhim.SubmitForm();
        return false;
    },
    ...
    ChangePageSize: function (e) {
        Common.QTG_QuyenTacGia_SearchPhim.SetPageSize($(e).val());
        Common.QTG_QuyenTacGia_SearchPhim.SubmitForm();
        return false;
    },
    ShowDialog: function () {
        $("#dialogSearchPhim").modal("show")
        Common.QTG_QuyenTacGia_SearchPhim.RegisterEvent();
    },
    HideDialog: function () {
    ...
    AddTrInTable: function (stt) {
        ...
        html += '<td class="text-center file-field" >'
            + '<button type = "button" class="btn btn-primary btn-block-sm btn-sm mt-md-3 mt-sm-3 uploadicon" onclick = "Common.QTG_QuyenTacGia_SearchPhim.UploadFileClick(this)"><i class="fas fa-upload"></i></button>'
        ...
        html += '<button title="lưu" class="btn btn-success btn-sm mr-1" type="button" onclick="' + "Common.QTG_QuyenTacGia_SearchPhim.SaveTrData(this," + stt + ",'Save')" + '"><i class="far fa-save"></i></button>';
        html += '<button title="hủy" class="btn btn-danger btn-sm" type="button" onclick="Common.QTG_QuyenTacGia_SearchPhim.DeleteTrData(this)"><i class="fa fa-times"></i></button>';
        html += '</td>';
    ...
    EditTrInTable: function (item, stt) {
        ...
        html += '<td class="text-center cursor" data-label="Thao tác">';
        html += '<button title="lưu" class="btn btn-success btn-sm mr-1" type="button" onclick="' + "Common.QTG_QuyenTacGia_SearchPhim.SaveTrData(this," + stt + ",'Save')" + '"><i class="far fa-save"></i></button>';
        html += '<button title="Xóa" class="btn btn-danger btn-sm" type="button" data-id="' + stt + '" onclick="Common.QTG_QuyenTacGia_SearchPhim.DeleteData(' + stt + ')"><i class="far fa-trash-alt"></i></button>';
        html += '</td>';
        return html;
    },
    UploadFileClick: function (that) {
        ...
        $('.form-control-file').unbind("change").change(function (ev) {
            ev.preventDefault();
            Common.QTG_QuyenTacGia_SearchPhim.UploadFileChange(this);
            return false;
        });
    },
    UploadFileChange: function (ev) {
        ...
        if (fileUploads.length > 0) {
            ...
            var sizeMB = 100;
            if (!regex.test(Common.QTG_QuyenTacGia_SearchPhim.GetTypeFile(fileName))) {
                $.notify({
                    // options
                ...
                Common.Ajax({
                    type: "POST",
                    url: Common.QTG_QuyenTacGia_SearchPhim.Url.UploadVideo,
                    dataType: 'json',
                    data: data,
                    contentType: false, // Not to set any content header
                    processData: false, // Not to process data
    ...
    SaveTrData: function (that, stt, action) {
        ...
        if (Common.QTG_QuyenTacGia_SearchPhim.ListPhim[stt]) {
            Common.QTG_QuyenTacGia_SearchPhim.ListPhim[stt].Ten = $.trim(tr.find("input[name='" + stt + "Ten']").val()) || '';
            Common.QTG_QuyenTacGia_SearchPhim.ListPhim[stt].CreatedUser = $.trim(tr.find("input[name='" + stt + "CreatedUser']").val()) || '';
            Common.QTG_QuyenTacGia_SearchPhim.ListPhim[stt].Tag = $.trim(tr.find("textarea[name='" + stt + "Tag']").val()) || '';
            Common.QTG_QuyenTacGia_SearchPhim.ListPhim[stt].GhiChu = $.trim(tr.find("textarea[name='" + stt + "GhiChu']").val()) || '';
            Common.QTG_QuyenTacGia_SearchPhim.ListPhim[stt].CreatedDate = $.trim(tr.find("input[name='" + stt + "CreatedDate']").val()) || '';
            Common.QTG_QuyenTacGia_SearchPhim.ListPhim[stt].TenTepTin = div.data("tenfile") || '';
            Common.QTG_QuyenTacGia_SearchPhim.ListPhim[stt].TenGoc = div.data("tenfilegoc") || '';
            Common.QTG_QuyenTacGia_SearchPhim.ListPhim[stt].DuongDan = div.data("duongdan") || '';
        }
        else {
            Common.QTG_QuyenTacGia_SearchPhim.ListPhim.push({
        ...
        //$.ajax({
        //    type: 'POST',
        //    url: Common.QTG_QuyenTacGia_SearchPhim.Url.SavePhim,
        //    data: formdata,
        //    datatype: "json",
        //    contentType: false, // Not to set any content header
        //    processData: false, // Not to process data
    ...
    BindDataUpdate: function (tr, model) {
        ...
        //add td
        let html = new Array();
        let row = Common.QTG_QuyenTacGia_SearchPhim.EditTrInTable(model, numberCurrent);
        html.push(row);
        tr.append(html);
...
var QTG_QuyenTacGia_SearchPhim = QTG_QuyenTacGia_SearchPhim;
QTG_QuyenTacGia_SearchPhim.constructor = QTG_QuyenTacGia_SearchPhim;
