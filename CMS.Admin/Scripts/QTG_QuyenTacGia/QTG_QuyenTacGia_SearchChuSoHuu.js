var QTG_QuyenTacGia_SearchChuSoHuu = function (options) {
    for (var property in options) {
        this[property] = options[property];
    }
    return this.Initialize(options);
}
QTG_QuyenTacGia_SearchChuSoHuu.prototype = {
    ...
    RegisterEvent: function () {
            ...
            if ($('input[name="chkInputCheckedCSH[]"]:checked').length > 0) {
                $('input[name="chkInputCheckedCSH[]"]').each(function () {
                    if ($(this).prop("checked")) {
                        var check = Common.QTG_QuyenTacGia_SearchChuSoHuu.ListChuSoHuu.find(x => x.ChuSoHuuID === $(this).closest("tr").data("id"));
                ...
                Common.QTG_QuyenTacGia_SearchChuSoHuu.LoadTableChuSoHuu();
                Common.QTG_QuyenTacGia_SearchChuSoHuu.HideDialog();
                return false;
            }
        });
        ...
        $(".btn-update-chusohuu").unbind("click").click(function () {
            ...
            Common.Ajax({
                url: Common.QTG_QuyenTacGia_SearchChuSoHuu.Url.UpdateChuSoHuu,
                type: "POST",
        ...
        $("#btnSaveChuSoHuu").unbind("click").click(function () {
            ...
            if ($("#form-update-chusohuu").find(".field-validation-error").length == 0) {
                ...
                $.ajax({
                    type: 'POST',
                    url: Common.QTG_QuyenTacGia_SearchChuSoHuu.Url.SaveChuSoHuu,
                    data: formdata,
                    datatype: "json",
    ...
    SubmitForm: function () {
        ...
        Common.Ajax({
            type: 'POST',
            url: Common.QTG_QuyenTacGia_SearchChuSoHuu.Url.SearchChuSoHuu,
            data: formdata,
            contentType: false, // Not to set any content header
            processData: false, // Not to process data
            dataType: 'html',
            ...
            Common.QTG_QuyenTacGia_SearchChuSoHuu.ShowDialog();
        });
        return false;
    },
    Paging: function (page) {
        Common.QTG_QuyenTacGia_SearchChuSoHuu.SetPage(page);
        Common.QTG_QuyenTacGia_SearchChuSoHuu.SubmitForm();
        return false;
    },
    ...
    ChangePageSize: function (e) {
        Common.QTG_QuyenTacGia_SearchChuSoHuu.SetPageSize($(e).val());
        Common.QTG_QuyenTacGia_SearchChuSoHuu.SubmitForm();
        return false;
    },
    ShowDialog: function () {
    ...
    LoadTableChuSoHuu: function () {
        var html = '';
        if (Common.QTG_QuyenTacGia_SearchChuSoHuu.ListChuSoHuu != null && Common.QTG_QuyenTacGia_SearchChuSoHuu.ListChuSoHuu.length > 0) {
            for (i = 0; i < Common.QTG_QuyenTacGia_SearchChuSoHuu.ListChuSoHuu.length; i++) {
                var chuSoHuu = Common.QTG_QuyenTacGia_SearchChuSoHuu.ListChuSoHuu[i];
                html += '<tr>';
                ...
                html += '<button title="Xóa chủ sở hữu" data-id="' + chuSoHuu.ChuSoHuuID + '" class="btn btn-danger btn-sm" type="button" onclick="Common.QTG_QuyenTacGia_SearchChuSoHuu.RemovedChuSoHuu(' + i + ')"><i class="far fa-trash-alt"></i></button>';
                html += '</td>';
                html += '</tr>';
            }
        }
    ...
    RemovedChuSoHuu: function (id) {
        Common.QTG_QuyenTacGia_SearchChuSoHuu.ListChuSoHuu.splice(id, 1)
        Common.QTG_QuyenTacGia_SearchChuSoHuu.LoadTableChuSoHuu();
        return false;
    },
};

var QTG_QuyenTacGia_SearchChuSoHuu = QTG_QuyenTacGia_SearchChuSoHuu;
QTG_QuyenTacGia_SearchChuSoHuu.constructor = QTG_QuyenTacGia_SearchChuSoHuu;
