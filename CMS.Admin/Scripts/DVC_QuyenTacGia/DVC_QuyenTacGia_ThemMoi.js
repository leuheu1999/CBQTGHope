var DVC_QuyenTacGia_ThemMoi = function (options) {
    for (var property in options) {
        this[property] = options[property];
    }
    return this.Initialize(options);
};
DVC_QuyenTacGia_ThemMoi.prototype = {
    ...
    RegisterEvent: function () {
        ...
        $("#VungMienID").unbind("change").change(function () {
            ...
            if (id > 0) {
                ...
                Common.Ajax({
                    url: Common.DVC_QuyenTacGia_ThemMoi.Url.CapSo,
                    type: "POST",
        ...
        $(".btnSubmitNewCapNhat").unbind('click').click(function (e) {
            ...
            if ($("#form-update-tt-tacpham").find(".field-validation-error").length == 0) {
                ...
                _model.ListHinhAnh = Common.DVC_QuyenTacGia_SearchHinhAnh.ListHinhAnh;
                _model.ListPhim = Common.DVC_QuyenTacGia_SearchPhim.ListPhim;
                _model.ListTacGia = Common.DVC_QuyenTacGia_SearchCongDan.ListTacGia;
                _model.ListChuSoHuu = Common.DVC_QuyenTacGia_SearchCongDan.ListChuSoHuu;
                _model.ListDinhKem = Common.DVC_QuyenTacGia_SearchDinhKem.ListDinhKem;
                ...
                $.ajax({
                    type: 'POST',
                    url: Common.DVC_QuyenTacGia_ThemMoi.Url.SaveQuyenTacGia,
                    data: formdata,
                    datatype: "json",
                ...
                Items: {
                    ...
                    Cancel: {
                        ...
                        OnClick: function (target) {
                            ...
                            if (key == 'capso')
                                window.location = Common.DVC_QuyenTacGia_ThemMoi.Url.IndexCapSo;
                            else if (id > 0)
                                window.location = Common.DVC_QuyenTacGia_ThemMoi.Url.IndexHoSo;
                            else
                                window.location = Common.DVC_QuyenTacGia_ThemMoi.Url.Index;
                        },
                        Value: "Đồng ý"
                    }
                }
        ...
        $(".btn-xoa-tacgia").unbind("click").click(function () {
            var id = $(this).data("id");
            Common.DVC_QuyenTacGia_SearchTacGia.RemovedTacGia(id);
            return false;
        });

        $(".btn-xoa-chusohuu").unbind("click").click(function () {
            var id = $(this).data("id");
            Common.DVC_QuyenTacGia_SearchChuSoHuu.RemovedChuSoHuu(id);
            return false;
        });

        $(".btn-xoa-tepdinhkem").unbind("click").click(function () {
            var id = $(this).data("id");
            Common.DVC_QuyenTacGia_SearchDinhKem.RemovedDinhKem(id);
            return false;
        });

        $(".btn-view-file").unbind("click").click(function () {
            var url = Common.DVC_QuyenTacGia_ThemMoi.Url.ViewFile
        ...
        $(".btnExportFile").unbind("click").click(function () {
            var form = $("#form-update-tt-tacpham");
            var _model = '"QuyenID":"' + form.find("input[name='QuyenTacGiaID']").val() + '",';
            _model += '"HoTenNguoiXuat":"' + form.find("input[name='UserFullName']").val() + '"';
            Common.Ajax({
                type: "POST",
                url: Common.DVC_QuyenTacGia_ThemMoi.Url.ExportWordPdf,
                async: false,
                cache: false,
                data: { Param: _model, Key: 'ExportDVC_QTGPhieuKiemSoat' }
            }, function (result) {
                if (result) {
...
var DVC_QuyenTacGia_ThemMoi = DVC_QuyenTacGia_ThemMoi;
DVC_QuyenTacGia_ThemMoi.constructor = DVC_QuyenTacGia_ThemMoi;