var QLQ_QuyenLienQuan_SearchPhim = function (options) {
    for (var property in options) {
        this[property] = options[property];
    }
    return this.Initialize(options);
}
QLQ_QuyenLienQuan_SearchPhim.prototype = {
    ...
    RegisterEvent: function () {
        ...
        $("#btnAddNewRowP").unbind("click").click(function (event) {
            ...
            tr.innerHTML = Common.QLQ_QuyenLienQuan_SearchPhim.AddTrInTable(numberCurrent);
        ...
        $(".btn-update-phim").unbind("click").click(function () {
            ...
            Common.QLQ_QuyenLienQuan_SearchPhim.BindDataUpdate(tr, Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim[id]);
            return false;
            //Common.ShowLoading(true);
            //Common.Ajax({
            //    url: Common.QLQ_QuyenLienQuan_SearchPhim.Url.UpdatePhim,
            //    type: "POST",
            ...
            //                Common.QLQ_QuyenLienQuan_SearchPhim.BindDataUpdate(tr, result.data);
            //                return false;
            //            }
            //        }
            //        else {
        ...
        $(".btn-xoa-phim").unbind("click").click(function () {
            ...
            Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim.splice(id, 1);
            Common.QLQ_QuyenLienQuan_SearchPhim.SubmitForm();
            return false;
            //Common.ShowLoading(true);
            //Common.Ajax({
            //    url: Common.QLQ_QuyenLienQuan_SearchPhim.Url.DeletePhim,
            //    type: "POST",
            //    ContentType: 'application/json; charset=utf-8',
            //    async: false,
            //    cache: false,
        ...
        $('.btn-chon-phim').unbind("click").click(function () {
            ...
            //$("#PhimID").val(id);
            $("#TenPhim").val(name);
            Common.QLQ_QuyenLienQuan_SearchPhim.HideDialog();
            return false;
        });
    },
    SubmitForm: function () {
        ...
        if (keyPage == 'update') {
            if (click >= 1) {
                var listDuLieu = Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim;
                var html = '';
                ...
                Common.QLQ_QuyenLienQuan_SearchPhim.ShowDialog();
                return false;
            }
            else {
                ...
                Common.Ajax({
                    type: 'POST',
                    url: Common.QLQ_QuyenLienQuan_SearchPhim.Url.SearchPhim,
                    data: formdata,
                    ...
                    $("#tableUpdatePhim tbody tr").each(function () {
                        if ($(this).closest("tr").data("id") >= 0) {
                            Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim.push({
    ...
    Paging: function (page) {
        Common.QLQ_QuyenLienQuan_SearchPhim.SetPage(page);
        Common.QLQ_QuyenLienQuan_SearchPhim.SubmitForm();
        return false;
    },
    ...
    ChangePageSize: function (e) {
        Common.QLQ_QuyenLienQuan_SearchPhim.SetPageSize($(e).val());
        Common.QLQ_QuyenLienQuan_SearchPhim.SubmitForm();
        return false;
    },
    ShowDialog: function () {
        $("#dialogSearchPhim").modal("show")
        Common.QLQ_QuyenLienQuan_SearchPhim.RegisterEvent();
    },
    HideDialog: function () {
    ...
    AddTrInTable: function (stt) {
        ...
        html += '<td class="text-center file-field" >'
            + '<button type = "button" class="btn btn-primary btn-block-sm btn-sm mt-md-3 mt-sm-3 uploadicon" onclick = "Common.QLQ_QuyenLienQuan_SearchPhim.UploadFileClick(this)"><i class="fas fa-upload"></i></button>'
        ...
        html += '<button title="lưu" class="btn btn-success btn-sm mr-1" type="button" onclick="' + "Common.QLQ_QuyenLienQuan_SearchPhim.SaveTrData(this," + stt + ",'Save')" + '"><i class="far fa-save"></i></button>';
        html += '<button title="hủy" class="btn btn-danger btn-sm" type="button" onclick="Common.QLQ_QuyenLienQuan_SearchPhim.DeleteTrData(this)"><i class="fa fa-times"></i></button>';
        html += '</td>';
    ...
    EditTrInTable: function (item, stt) {
        ...
        html += '<td class="text-center cursor" data-label="Thao tác">';
        html += '<button title="lưu" class="btn btn-success btn-sm mr-1" type="button" onclick="' + "Common.QLQ_QuyenLienQuan_SearchPhim.SaveTrData(this," + stt + ",'Save')" + '"><i class="far fa-save"></i></button>';
        html += '<button title="Xóa" class="btn btn-danger btn-sm" type="button" data-id="' + stt + '" onclick="Common.QLQ_QuyenLienQuan_SearchPhim.DeleteData(' + stt + ')"><i class="far fa-trash-alt"></i></button>';
        html += '</td>';
        return html;
    },
    UploadFileClick: function (that) {
        ...
        $('.form-control-file').unbind("change").change(function (ev) {
            ev.preventDefault();
            Common.QLQ_QuyenLienQuan_SearchPhim.UploadFileChange(this);
            return false;
        });
    },
    UploadFileChange: function (ev) {
        ...
        if (fileUploads.length > 0) {
            ...
            var sizeMB = 100;
            if (!regex.test(Common.QLQ_QuyenLienQuan_SearchPhim.GetTypeFile(fileName))) {
                $.notify({
                    // options
                ...
                Common.Ajax({
                    type: "POST",
                    url: Common.QLQ_QuyenLienQuan_SearchPhim.Url.UploadVideo,
                    dataType: 'json',
                    data: data,
                    contentType: false, // Not to set any content header
                    processData: false, // Not to process data
    ...
    SaveTrData: function (that, stt, action) {
        ...
        if (Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim[stt]) {
            Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim[stt].Ten = $.trim(tr.find("input[name='" + stt + "Ten']").val()) || '';
            Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim[stt].CreatedUser = $.trim(tr.find("input[name='" + stt + "CreatedUser']").val()) || '';
            Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim[stt].Tag = $.trim(tr.find("textarea[name='" + stt + "Tag']").val()) || '';
            Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim[stt].GhiChu = $.trim(tr.find("textarea[name='" + stt + "GhiChu']").val()) || '';
            Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim[stt].CreatedDate = $.trim(tr.find("input[name='" + stt + "CreatedDate']").val()) || '';
            Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim[stt].TenTepTin = div.data("tenfile") || '';
            Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim[stt].TenGoc = div.data("tenfilegoc") || '';
            Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim[stt].DuongDan = div.data("duongdan") || '';
        }
        else {
            Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim.push({
        ...
        //if (Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim[stt])
        //    Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim.splice(stt, 1);
        //Common.QLQ_QuyenLienQuan_SearchPhim.ListPhim.push({
        //    Ten: $.trim(tr.find("input[name='" + stt + "Ten']").val()) || '',
        //    CreatedUser: $.trim(tr.find("input[name='" + stt + "CreatedUser']").val()) || '',
        //    Tag: $.trim(tr.find("textarea[name='" + stt + "Tag']").val()) || '',
        ...
        //$.ajax({
        //    type: 'POST',
        //    url: Common.QLQ_QuyenLienQuan_SearchPhim.Url.SavePhim,
        //    data: formdata,
        //    datatype: "json",
        //    contentType: false, // Not to set any content header
        //    processData: false, // Not to process data
    ...
    BindDataUpdate: function (tr, model) {
        ...
        //add td
        let html = new Array();
        let row = Common.QLQ_QuyenLienQuan_SearchPhim.EditTrInTable(model, numberCurrent);
        html.push(row);
        tr.append(html);
...
var QLQ_QuyenLienQuan_SearchPhim = QLQ_QuyenLienQuan_SearchPhim;
QLQ_QuyenLienQuan_SearchPhim.constructor = QLQ_QuyenLienQuan_SearchPhim;
