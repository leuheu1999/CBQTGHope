var QLQ_QuyenLienQuan_SearchDinhKem = function (options) {
    for (var property in options) {
        this[property] = options[property];
    }
    return this.Initialize(options);
}
QLQ_QuyenLienQuan_SearchDinhKem.prototype = {
    ...
    RegisterEvent: function () {
        ...
        $("#btnAddNewRowDK").unbind("click").click(function (event) {
            ...
            tr.innerHTML = Common.QLQ_QuyenLienQuan_SearchDinhKem.AddTrInTable(numberCurrent);
        ...
        $(".btn-update-dinhkem").unbind("click").click(function () {
            ...
            Common.Ajax({
                url: Common.QLQ_QuyenLienQuan_SearchDinhKem.Url.UpdateDinhKem,
                type: "POST",
                ...
                if (!Common.Empty(result)) {
                    if (result.status == true) {
                        if (!Common.Empty(result.data)) {
                            Common.QLQ_QuyenLienQuan_SearchDinhKem.BindDataUpdate(tr, result.data);
                            return false;
                        }
                    }
                    else {
        ...
        $(".btn-xoa-dinhkem").unbind("click").click(function () {
            ...
            Common.ShowLoading(true);
            Common.Ajax({
                url: Common.QLQ_QuyenLienQuan_SearchDinhKem.Url.DeleteDinhKem,
                type: "POST",
                ContentType: 'application/json; charset=utf-8',
                async: false,
                cache: false,
            ...
            if ($('input[name="chkInputCheckedDK[]"]:checked').length > 0) {
                $('input[name="chkInputCheckedDK[]"]').each(function () {
                    if ($(this).prop("checked")) {
                        var check = Common.QLQ_QuyenLienQuan_SearchDinhKem.ListDinhKem.find(x => x.DinhKemID === $(this).closest("tr").data("id"));
                ...
                Common.QLQ_QuyenLienQuan_SearchDinhKem.LoadTableDinhKem();
                Common.QLQ_QuyenLienQuan_SearchDinhKem.HideDialog();
                return false;
            }
        });
    },
    SubmitForm: function () {
        ...
        Common.Ajax({
            type: 'POST',
            url: Common.QLQ_QuyenLienQuan_SearchDinhKem.Url.SearchDinhKem,
            data: formdata,
            ...
            Common.QLQ_QuyenLienQuan_SearchDinhKem.ShowDialog();
        });
        return false;
    },
    Paging: function (page) {
        Common.QLQ_QuyenLienQuan_SearchDinhKem.SetPage(page);
        Common.QLQ_QuyenLienQuan_SearchDinhKem.SubmitForm();
        return false;
    },
    ...
    ChangePageSize: function (e) {
        Common.QLQ_QuyenLienQuan_SearchDinhKem.SetPageSize($(e).val());
        Common.QLQ_QuyenLienQuan_SearchDinhKem.SubmitForm();
        return false;
    },
    ShowDialog: function () {
    ...
    AddTrInTable: function (stt) {
        ...
        html += '<td class="text-center file-field" >'
            + '<button type = "button" class="btn btn-primary btn-block-sm btn-sm mt-md-3 mt-sm-3 uploadicon" onclick = "Common.QLQ_QuyenLienQuan_SearchDinhKem.UploadFileClick(this)"><i class="fas fa-upload"></i></button>'
        ...
        html += '<button title="lưu" class="btn btn-success btn-sm mr-2" type="button" onclick="' + "Common.QLQ_QuyenLienQuan_SearchDinhKem.SaveTrData(this," + stt + ",'Save')" + '"><i class="far fa-save"></i></button>';
        html += '<button title="hủy" class="btn btn-danger btn-sm" type="button" onclick="Common.QLQ_QuyenLienQuan_SearchDinhKem.DeleteTrData(this)"><i class="fa fa-times"></i></button>';
        html += '</td>';
    ...
    UploadFileClick: function (that) {
        ...
        $('.form-control-file').unbind("change").change(function (ev) {
            ev.preventDefault();
            Common.QLQ_QuyenLienQuan_SearchDinhKem.UploadFileChange(this);
            return false;
        });
    },
    UploadFileChange: function (ev) {
        ...
        if (fileUploads.length > 0) {
            ...
            var sizeMB = 20;
            if (!regex.test(Common.QLQ_QuyenLienQuan_SearchDinhKem.GetTypeFile(fileName))) {
                $.notify({
                    // options
                ...
                Common.Ajax({
                    type: "POST",
                    url: Common.QLQ_QuyenLienQuan_SearchDinhKem.Url.UploadFile,
                    dataType: 'json',
                    data: data,
                    contentType: false, // Not to set any content header
                    processData: false, // Not to process data
    ...
    SaveTrData: function (that, stt, action) {
        ...
        $.ajax({
            type: 'POST',
            url: Common.QLQ_QuyenLienQuan_SearchDinhKem.Url.SaveDinhKem,
            data: formdata,
            datatype: "json",
            contentType: false, // Not to set any content header
            processData: false, // Not to process data
    ...
    BindDataUpdate: function (tr, model) {
        ...
        //add td
        let html = new Array();
        let row = Common.QLQ_QuyenLienQuan_SearchDinhKem.EditTrInTable(model, numberCurrent);
        html.push(row);
        tr.append(html);
    ...
    LoadTableDinhKem: function () {
        var html = '';
        if (Common.QLQ_QuyenLienQuan_SearchDinhKem.ListDinhKem != null && Common.QLQ_QuyenLienQuan_SearchDinhKem.ListDinhKem.length > 0) {
            for (i = 0; i < Common.QLQ_QuyenLienQuan_SearchDinhKem.ListDinhKem.length; i++) {
                var dinhKem = Common.QLQ_QuyenLienQuan_SearchDinhKem.ListDinhKem[i];
                ...
                if (dinhKem.IsMotCua)
                    ...
                    html += '<button title="Xóa đính kèm" data-id="' + dinhKem.DinhKemID + '" class="btn btn-danger btn-sm" type="button" onclick="Common.QLQ_QuyenLienQuan_SearchDinhKem.RemovedDinhKem(' + i + ')"><i class="far fa-trash-alt"></i></button>';
                html += '</td>';
                html += '</tr>';
            }
        }
    ...
    RemovedDinhKem: function (id) {
        Common.QLQ_QuyenLienQuan_SearchDinhKem.ListDinhKem.splice(id, 1)
        Common.QLQ_QuyenLienQuan_SearchDinhKem.LoadTableDinhKem();
        return false;
    },
};

var QLQ_QuyenLienQuan_SearchDinhKem = QLQ_QuyenLienQuan_SearchDinhKem;
QLQ_QuyenLienQuan_SearchDinhKem.constructor = QLQ_QuyenLienQuan_SearchDinhKem;
